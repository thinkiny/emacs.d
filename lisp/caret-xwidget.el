...
(defun caret-xwidget-translate-word ()
  ...
  (interactive)
  (caret-xwidget--exec
   (concat caret-xwidget--js-prefix "caretInfo()")
   (lambda (result)
     (when (and result (not (string-empty-p result)))
       (let* ((data (json-parse-string result))
              (edges (window-inside-pixel-edges))
              (word (gethash "text" data))
              (x (+ (nth 0 edges) (round (gethash "left" data))))
              (y (+ (nth 1 edges) (round (gethash "bottom" data)))))
...
(with-eval-after-load 'xwidget
  ...
  (define-key xwidget-webkit-mode-map (kbd "l")     #'caret-xwidget-forward-word)
  ...
  (define-key xwidget-webkit-mode-map (kbd "(")     #'caret-xwidget-backward-sentence)
